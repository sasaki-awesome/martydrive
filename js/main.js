...
/* ---------- localStorage から商品データを読み込む ----------
   管理画面で保存したデータを優先し、なければデフォルトを使う */
...
function openModal(id) {
  ...
  /* 左カラム：画像または絵文字×2 */
  document.getElementById("m-imgs").innerHTML = p.img
    ? `<div class="m-img-main" style="overflow:hidden;">
         <img src="${p.img}" alt="${p.name}" style="width:100%;height:100%;object-fit:cover;border-radius:10px;">
       </div>
       <div class="m-img-sub" style="overflow:hidden;">
         <img src="${p.img}" alt="${p.name}" style="width:100%;height:100%;object-fit:cover;border-radius:10px;opacity:.7;">
       </div>`
    : `<div class="m-img-main">${p.e1}</div>
       <div class="m-img-sub">${p.e2}</div>`;

  document.getElementById("m-info").innerHTML = `
    <div class="m-cat">${CAT[p.cat] || p.cat}</div>
